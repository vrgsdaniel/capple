---
description: "Architectural review of new features. Use when: a feature branch or set of files has been implemented and needs scanning for inconsistencies, anti-patterns, or deviations from project conventions before merging."
...
List every file that is new or modified for this feature. If the user has not specified them, scan `git diff --name-only HEAD` or ask the user to list them.
...
- [ ] No `supabase_realtime` publication missing for tables requiring live sync
...
- [ ] `MagicMock(spec=GroceryService)` (or relevant service) is used — not bare `MagicMock()`
