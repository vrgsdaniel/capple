...
class TestListItems:
    async def test_returns_active_and_history(self, service, mock_db):
        ...
        result = await service.list_items(FAKE_USER_ID)

        assert len(result["active"]) == 1
        assert len(result["history"]) == 1

    async def test_enriches_source_recipe_title(self, service, mock_db):
        mock_db.find_grocery_items.side_effect = [
            [],
            [dict(FAKE_BOUGHT_ITEM)],
        ]
    ...
    async def test_source_recipe_title_is_none_when_no_recipe(self, service, mock_db):
        item = dict(FAKE_ITEM)
    ...
    async def test_raises_when_no_household(self, service, mock_db):
        ...
        with pytest.raises(NotFoundException):
            await service.list_items(FAKE_USER_ID)


class TestAddItem:
    async def test_creates_new_item(self, service, mock_db):
        mock_db.find_active_grocery_item.return_value = None
        mock_db.create_grocery_item.return_value = dict(FAKE_ITEM)

        await service.add_item(FAKE_USER_ID, "Milk", "2 L")
    ...
    async def test_merges_qty_when_item_already_active(self, service, mock_db):
        ...
        await service.add_item(FAKE_USER_ID, "Milk", "1 L")
    ...
    async def test_merge_appends_with_plus_for_different_units(self, service, mock_db):
        existing = {**FAKE_ITEM, "qty": "200 g"}
        mock_db.find_active_grocery_item.return_value = existing
        mock_db.update_grocery_item.return_value = existing

        await service.add_item(FAKE_USER_ID, "Flour", "1 cup")

        call_data = mock_db.update_grocery_item.call_args[0][1]
        assert call_data["qty"] == "200 g + 1 cup"

    async def test_normalises_name_for_duplicate_check(self, service, mock_db):
        mock_db.find_active_grocery_item.return_value = None
        mock_db.create_grocery_item.return_value = dict(FAKE_ITEM)

        await service.add_item(FAKE_USER_ID, "  MILK  ", None)
...
class TestAddFromRecipe:
    async def test_creates_items_with_source_recipe_id(self, service, mock_db):
        mock_db.find_active_grocery_item.return_value = None
        mock_db.create_grocery_item.return_value = dict(FAKE_ITEM)
        ingredients = [{"name": "Milk", "qty": "200 ml"}, {"name": "Eggs", "qty": "2"}]

        await service.add_from_recipe(FAKE_USER_ID, FAKE_RECIPE_ID, ingredients)
    ...
    async def test_merges_ingredient_into_existing_active_item(self, service, mock_db):
        existing = dict(FAKE_ITEM)  # Milk, 2 L
        mock_db.find_active_grocery_item.return_value = existing
        mock_db.update_grocery_item.return_value = existing

        await service.add_from_recipe(FAKE_USER_ID, FAKE_RECIPE_ID, [{"name": "Milk", "qty": "1 L"}])

        mock_db.update_grocery_item.assert_called_once()
        mock_db.create_grocery_item.assert_not_called()
        assert mock_db.update_grocery_item.call_args.kwargs["household_id"] == FAKE_HOUSEHOLD_ID

    async def test_skips_ingredients_with_empty_name(self, service, mock_db):
        mock_db.create_grocery_item.return_value = dict(FAKE_ITEM)

        results = await service.add_from_recipe(FAKE_USER_ID, FAKE_RECIPE_ID, [{"name": "", "qty": "1"}])

        assert results == []
        mock_db.create_grocery_item.assert_not_called()

    async def test_returns_all_affected_items(self, service, mock_db):
        mock_db.find_active_grocery_item.return_value = None
        ...
        results = await service.add_from_recipe(FAKE_USER_ID, FAKE_RECIPE_ID, [{"name": "Milk"}, {"name": "Eggs"}])

        assert len(results) == 2


class TestMarkBought:
    async def test_marks_item_as_bought(self, service, mock_db):
        updated = {**FAKE_ITEM, "bought": True}
        mock_db.update_grocery_item.return_value = updated

        result = await service.mark_bought(FAKE_USER_ID, FAKE_ITEM["id"])
    ...
    async def test_raises_when_item_not_found(self, service, mock_db):
        mock_db.update_grocery_item.return_value = None

        with pytest.raises(NotFoundException):
            await service.mark_bought(FAKE_USER_ID, "nonexistent-id")


class TestRestoreItem:
    async def test_restores_bought_item_to_active(self, service, mock_db):
        ...
        result = await service.restore_item(FAKE_USER_ID, bought["id"])
    ...
    async def test_merges_into_existing_active_item_on_restore(self, service, mock_db):
        ...
        await service.restore_item(FAKE_USER_ID, bought["id"])
    ...
    async def test_raises_when_merge_target_missing_during_restore(self, service, mock_db):
        bought = {**FAKE_BOUGHT_ITEM, "name": "Milk", "qty": "1 L"}
        active_match = {**FAKE_ITEM, "name": "Milk", "qty": "2 L"}
        mock_db.get_grocery_item.return_value = bought
        mock_db.find_active_grocery_item.return_value = active_match
    ...
    async def test_raises_when_restore_update_affects_zero_rows(self, service, mock_db):
        bought = dict(FAKE_BOUGHT_ITEM)
        mock_db.get_grocery_item.return_value = bought
        mock_db.find_active_grocery_item.return_value = None
        mock_db.update_grocery_item.return_value = None
    ...
    async def test_raises_when_item_not_found(self, service, mock_db):
        ...
        with pytest.raises(NotFoundException):
            await service.restore_item(FAKE_USER_ID, "nonexistent-id")


class TestRemoveItem:
    async def test_deletes_item(self, service, mock_db):
        mock_db.delete_grocery_item.return_value = True

        await service.remove_item(FAKE_USER_ID, FAKE_ITEM["id"])
    ...
    async def test_raises_when_item_not_found(self, service, mock_db):
        ...
        with pytest.raises(NotFoundException):
            await service.remove_item(FAKE_USER_ID, "nonexistent-id")


class TestClearHistory:
    async def test_deletes_all_bought_items(self, service, mock_db):
        await service.clear_history(FAKE_USER_ID)
