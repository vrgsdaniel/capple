...
class TestCreateBatteryLog:
    async def test_creates_log_with_household(self, service, mock_db):
        mock_db.get_household_by_user.return_value = FAKE_HOUSEHOLD
        mock_db.create_battery_log.return_value = FAKE_LOG

        result = await service.create_battery_log(FAKE_USER_ID, level=75, note="Feeling good", effective_at="2026-05-01T10:00:00+00:00")
    ...
    async def test_raises_not_found_when_no_household(self, service, mock_db):
        mock_db.get_household_by_user.return_value = None

        with pytest.raises(NotFoundException):
            await service.create_battery_log(FAKE_USER_ID, level=75, note=None, effective_at="2026-05-01T10:00:00+00:00")


class TestUpdateBatteryLog:
    async def test_updates_own_log(self, service, mock_db):
        updated = {**FAKE_LOG, "level": 50}
        mock_db.update_battery_log.return_value = updated

        result = await service.update_battery_log(FAKE_USER_ID, "log-001", {"level": 50})

        assert result["level"] == 50
        mock_db.update_battery_log.assert_called_once_with("log-001", FAKE_USER_ID, {"level": 50})

    async def test_raises_not_found_when_no_rows_affected(self, service, mock_db):
        mock_db.update_battery_log.return_value = None

        with pytest.raises(NotFoundException):
            await service.update_battery_log(FAKE_USER_ID, "log-001", {"level": 50})

    async def test_raises_not_found_when_empty_updates(self, service, mock_db):
        with pytest.raises(NotFoundException):
            await service.update_battery_log(FAKE_USER_ID, "log-001", {})


class TestDeleteBatteryLog:
    async def test_deletes_own_log(self, service, mock_db):
        mock_db.delete_battery_log.return_value = True

        await service.delete_battery_log(FAKE_USER_ID, "log-001")
...
class TestGetHouseholdBatteryLogs:
    async def test_returns_logs_for_household(self, service, mock_db):
        mock_db.get_household_by_user.return_value = FAKE_HOUSEHOLD
        mock_db.find_battery_logs_by_household.return_value = [FAKE_LOG]

        result = await service.get_household_battery_logs(FAKE_USER_ID, "2026-05-01T00:00:00Z", "2026-05-02T00:00:00Z")
    ...
    async def test_raises_not_found_when_no_household(self, service, mock_db):
        ...
        with pytest.raises(NotFoundException):
            await service.get_household_battery_logs(FAKE_USER_ID, "2026-05-01T00:00:00Z", "2026-05-02T00:00:00Z")
