...
class _FakeDB:
    ...
    async def find_battery_logs_by_household(self, household_id, since, end):
...
async def test_get_battery_tool_builds_expected_summary():
    logs = [
    ...
    result = await battery_tool.build_battery_tool(
...
async def test_get_battery_tool_prefers_runtime_context_values():
    logs = [
...
async def test_create_get_battery_tool_tool_returns_invokable_tool():
    logs = [
        {"user_id": "u1", "level": 40, "effective_at": "2026-05-16T09:00:00+00:00"},
        {"user_id": "u2", "level": 70, "effective_at": "2026-05-16T10:00:00+00:00"},
    ]
