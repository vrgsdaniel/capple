...
class TaskService:
    ...
    def list_tasks(self, user_id: str, page: int = 1, page_size: int = 20, include_history: bool = True) -> dict:
        ...
        active, active_total, history, history_total = self.db.find_tasks_with_count(
    ...
    def update_task(self, user_id: str, task_id: str, updates: dict) -> dict:
        household_id = self._resolve_household(user_id)
        ...
        result = self.db.update_task(task_id, updates, household_id=household_id, active_only=True)
        if not result:
    ...
    def delete_task(self, user_id: str, task_id: str) -> None:
        ...
        deleted = self.db.delete_task(task_id, household_id=household_id, active_only=True)
        if not deleted:
            log.warning(f"Task {task_id} not found or already completed in household {household_id}")
            raise NotFoundException("Task not found or already completed.")
